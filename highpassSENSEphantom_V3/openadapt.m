...
%   2010       Michael V?lker - improved computation speed
...
%   2011       Michael V?lker - speed increase on multicore due to parfor-loops
...
%
%   2012       Weick / V?lker - 2D and 3D possible
%              Michael V?lker - updated help text and improved error checking
    ...
    for y = 1:NstepY      % different y-coordinates are treated simultaneously on several workers
        ...
        for x = 1:NstepX
            ...
            for z = 1:NstepZ
                ...
                if bNoise
                    WsmTmp(:,x,z) = normmf / (normmf' * (Rn \ normmf));   % MiV?: Should the denomninator not be sqrt(normmf' * (Rn \ normmf)) ? --> Eq. (20) in the Walsh-Paper
                else
                    WsmTmp(:,x,z) = normmf;
                end
