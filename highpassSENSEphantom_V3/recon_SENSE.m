...
save('central_kdata','central_kdata');
...
% filename_sense='central_kdata.mat'; datatype =2;
filename_highpass='full_kspace_data.mat'; datatype =2;
filename_sense='full_kspace_data.mat'; datatype =2;
...
% Filters.type ='wavelet';  Filters.wavelet_name = 'bior5.5'; Filters.wavelet_levels=3; % for wavelet
 ...
 [channelsensitivity, ROI_mask]= sensitivity_estimation(filename_sense, datatype, Npe_tobe,Filters,Rnoise); %sense sensitivity
 
 [channelsensitivity_highpass, ROI_mask]= sensitivity_estimation(filename_highpass, datatype, Npe_tobe,Filters,Rnoise); %sense sensitivity
...
[recon_sense_highpassfiltered, flag,gmap] = sense(channelsensitivity_highpass, highpassed_reduced_kspace_data, FOV_reduction_factor); %high-pass SENSE
...
error_image_HIGHPASSSENSEsubSENSE=abs(abs(recon_HIGHPASSSENSE)-recon_sense);
...
figure(7); imagesc(abs(recon_sense_highpassfiltered)); colormap('gray'); axis square; colorbar;title('recon_sense_highpassfiltered'); 
figure(8); imagesc(error_image_HIGHPASSSENSEsubSENSE); colormap('gray'); axis square; colorbar; title(sprintf('L2 recon error: %s',L2diff_error_image_HIGHPASSSENSEsubSENSE));
